 ...
 * Ensures "Sales Consultant" appears at the bottom of the table (like the screenshot).
...
function createEmailContent(data) {
  ...
  // Override display labels for prettier names
  const labelMap = {
  ...
  const toTitle = (key) => {
    ...
    // turn camelCase or snake_case into Title Case
  ...
  // Collect regular rows and hold Sales Consultant to append last
  const regularRows = [];
  let salesConsultantRow = null;
  ...
  Object.keys(data)
    ...
    .forEach((k) => {
      ...
      const row = [toTitle(k), value];

      if (k === "salesConsultant") {
        salesConsultantRow = row; // keep for the bottom
      } else {
        regularRows.push(row);
      }
    });

  // Append Sales Consultant at the very end if present
  if (salesConsultantRow) regularRows.push(salesConsultantRow);
  ...
  const textBody = regularRows.map(([k, v]) => `${k}: ${v}`).join("\n");
